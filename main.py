...
def check_in_db(chat_id,product_link):
    with Session(engine) as session:
        q = session.query(prices).filter(prices.c.chat_id==chat_id).filter(prices.c.product_link==product_link)
...
def deleteFromDb(chat_id,message_id):
    with Session(engine) as session:
        delete_stmt = delete(prices).where(prices.c.chat_id == chat_id and prices.c.message_id == message_id)
...
def updateInDb(chat_id,product_link,product_price,lowest_price):
    with Session(engine) as session:
        update_stmt = prices.update().where(prices.c.chat_id == chat_id and prices.c.product_link == product_link).values(product_price=product_price,lowest_price=lowest_price)
...
def check_price_drop():
    ...
    for item in items:
        ...
        if current_price < item.lowest_price:
            ...
            keyboard = [[ InlineKeyboardButton("Product Page",url=item.product_link), InlineKeyboardButton("Stop alerts for ☝️",callback_data=item.message_id)]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            bot.send_message(item.chat_id,f'Price for {title} has dropped to ₹ {current_price}.',reply_markup=reply_markup)
            updateInDb(item.chat_id,item.product_link,current_price,lowest_price)
        else:
            print("😔")
...
def get_price(link):
    ...
    price = soup.find_all("span", {"class":"a-price-whole"})[0].get_text().replace(',', '').replace('₹', '').replace(' ', '').replace('.','').strip()
...
def onMsgReceived(update: Update,context: CallbackContext):
    if update.message.text:
        ...
        msg_id=context.bot.send_message(chat_id,"Please wait while I checkout this product's price 🙂").message_id
        ...
        if re.search(amazonRegex,msg):
            ...
            insertInDb(user_name=user_name,chat_id=chat_id,product_link=msg,product_name=title,product_price=current_price,lowest_price=current_price,message_id=message_id,availability=available)
